...
def opencode_session_ids(stdout):
    ...
    for line in stdout.splitlines():
        ...
        if event.get('type') not in ('step_start', 'text', 'step_finish', 'error'):
            unusable += 1
        if event.get('type') in ('step_start', 'text', 'step_finish') and 'part' not in event:
            unusable += 1
...
def run_trial(driver, *, prompt, marker=None, state='idle', settle=None,
    ...
    def clock_drift():
        ...
        # Each wall reading happened inside its monotonic bracket. Descheduling widens
        # uncertainty; only divergence outside every possible elapsed comparison proves a step.
        ...
        if drift_low > CLOCK_DRIFT_TOLERANCE or drift_high < -CLOCK_DRIFT_TOLERANCE:
            return drift_low if drift_low > CLOCK_DRIFT_TOLERANCE else drift_high
        return None
    ...
    if accepted is False:
        ...
        observable = {'accepted': clock_step is None}
